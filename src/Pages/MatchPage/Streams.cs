    ...
    public static class Streams {
        public static void Show(HtmlNode docNode, bool notOver) {
            ...
            if (notOver) {
                ...
                HtmlNodeCollection streamBoxes = linkNode.SelectNodes(".//div[@class=\"stream-box \"]");

                if (streamBoxes == null)
                    Console.WriteLine("No streams available");
                else {
                    foreach(HtmlNode streamBox in streamBoxes) {
                        HtmlNode embed = streamBox.SelectSingleNode(".//div[@class=\"stream-box-embed\"]");
                        string streamer = embed.InnerText;
                        HtmlNode extStream = streamBox.SelectSingleNode(".//div[@class=\"external-stream\"]");
                        HtmlNode linkButton = extStream.SelectSingleNode(".//a");
                        string url = linkButton.GetAttributeValue("href", "URL unavailable");
                        Console.WriteLine(string.Format("{0,-15}: {1}", streamer, url));
                    }
                }
